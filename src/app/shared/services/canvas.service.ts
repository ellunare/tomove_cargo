...
export class CanvasService {
	...
	_getOrientation(file) {    // Ориентация
	...
	_dataURItoBlob(dataURI) {    // Получаем Blob из финального canvas-dataURI
		// base64 --> binary raw
		let byteString
	...
	async _loadImage(file) {    // Получение URL и инфо фото из исходного файла
	...
	async _drawCanvas(files, canvas_) {    // Получение финальной панорамы
		...
		for (let p of data.pictures) {
			...
			if (p.orientation === 6 || p.orientation === 8 || p.orientation === 3) {  ////////////// ---- Повернуть
			...
			// ctx.font = "60px Arial"
			// ctx.fillStyle = "red"
			// ctx.fillText(p.orientation, start + 50, 100)
			/////////////////////////////

			start += p.addwidth
		}

		let result_canvas = canvas.toDataURL('image/jpeg', 0.5)
